...
class Strategy5_3:
    ...
    async def load_params(self):
        params = await self.interface.get_strategy_params('test-5_3')
        if not params:
    ...
    async def run_checks(self, params, signal, current_price):
        ...
        if direction == "long" and lr_angle <= Decimal("0.008"):
            logging.warning(f"lr_angle = {lr_angle}, слишком плоский или нисходящий для LONG. Вход отменён.")
            return False

        if direction == "short" and lr_angle >= Decimal("-0.008"):
