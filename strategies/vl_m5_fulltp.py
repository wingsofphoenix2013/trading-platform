### Стратегия vl_m5_fulltp — реверсивная логика входа и выхода

"""
Стратегия vl_m5_fulltp: реверсивная логика входов по управляющему сигналу.
Работает в обе стороны (long/short). При каждом сигнале открывает позицию в его направлении.
Если включён стоп-лосс — рассчитывает его и фиксирует в журнале. ТП в один прием.
"""
...
async def process_signal(log_id: int):
    print(f"[STRATEGY] vl_m5_fulltp: запуск обработки log_id={log_id}", flush=True)
    ...
    try:
        ...
        tp_offset = (entry_price * Decimal("0.0035")).quantize(Decimal(f'1e-{precision_price}'), rounding=ROUND_DOWN)
